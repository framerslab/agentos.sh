---
title: "Paracosm: why we built a structured world-model engine for LLM agent swarms, and the wilds.ai vault that made it inevitable"
date: "2026-05-05"
featured: true
excerpt: "Paracosm is an open-source TypeScript engine our team built that compiles a natural-language scenario into a typed deterministic world and runs HEXACO-typed AI agents through it. Same world, same kernel, same seed, divergent events and futures. This is why we created it, what it does, the moment in wilds.ai when LLM-driven NPCs locked themselves in a vault to survive a fight I started and stranded me in a desert with no recourse, and the technical details for the people who want to read them."
author: "Johnny Dunn"
...
The thing that finally pushed me to start building Paracosm was getting locked out of a vault by my own product.

I was inside [wilds.ai](https://wilds.ai), the AI-companion thing I lead, doing what I always do when a system feels too well-behaved: trying to break it. I picked a fight. I told the NPCs I was going to attack. The NPCs did not roll initiative. They did not parse my aggression as a combat trigger. They evaluated, the way LLMs do, and made a strategic move I did not see coming: they retreated into a vault and sealed it behind them. The vault locked them in with limited food and water. It locked me out in a vast unfamiliar desert with neither.

By the time I understood what had happened, there was no one to talk to. No NPC to apologize to, no character to ask for help, no path to recourse. I was wandering an end-to-end desert in a region I had never been with no supplies and no narrative thread. The NPCs were probably running down a clock inside the vault, having decided that locked-in starvation on a slow timeline was a better expected value than being attacked on a fast one. I was probably running down a faster clock outside, having decided nothing because I had nothing to decide with. The two outcomes were correlated and unobservable. I was never going to know which of us made it past sundown.

That was the moment. It was not "NPC refuses combat." It was "LLM evaluates a multi-step strategic move, executes it, and produces an emergent failure mode neither of us saw coming." Emergent behavior is what we are supposed to want. The problem was that I, the player, had no idea whether the NPCs had picked the locally optimal short-term move at the cost of a long-term doom they had not yet computed, no way to fork the world from the moment of declared aggression and rerun it with a different leader's HEXACO profile, no way to learn anything from the trajectory because the trajectory had no replay button.

That's the gap. The companion was alive enough to make a decision I did not predict. It was not legible enough for me to *learn anything from* the decision.
...
The rest of this post is why a game designer who works as an applied AI engineer thinks the next step in human-computer interaction is structured, replayable simulation, what the published academic and industrial work calls a "world model" and why half of those conversations are talking past each other, how Paracosm fits into the second half, and the technical details for the people who want them. There are arxiv links because I want you to follow them. There is a live demo at the bottom because I want you to use it. There is a fight that ended with both sides locked into different rooms of the same disaster.
...
Back to the wilds.ai fight that didn't happen. The NPCs that retreated were not broken. They were running the agent loop the wilds-ai engine has shipped since launch: read the conversation, infer the user's intent, decide what action serves the character's continuity, execute. The characters were non-combatant residents in an [active fantasy quest](https://github.com/framersai/wilds-ai/tree/master/packages/wilds-games/src/curated/campaign-fantasy). When I declared aggression the agent's policy router did exactly what the design said: it preserved character integrity over playing along with the player's framing. The vault was the locally optimal play. The LLM saw an attacker, weighed the cost of fighting against the cost of hiding, and locked the door.
...
The NPCs that locked themselves in a vault to survive a fight I started were the first time I had a system make a decision I could not have predicted and could not learn from. Paracosm is the second time. The difference is the second time I get the trajectory.
